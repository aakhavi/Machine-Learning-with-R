...
author: "Arash Akhavi & Nathan Diekema"
date: "11/26/2021"
output:
  rmdformats::downcute:
    lightbox: true
...
The first plot (PC1 vs PC2) is very effective at seperating the mammal and fish classes but does not perform as well on the other classes. The second plot (PX2 vs PC3) effectively separates the bird class, is okay at separating the invertebrate, but the rest are muddled together.
...
svm_mod <- svm_poly(cost = 30.751890537, degree = 1) %>%
  set_mode("classification") %>%
  set_engine("kernlab")
...
As earlier there is a greater emphasis on the roc_auc metric as well as the sensitivity and specificity metrics over the accuracy. We can see from the table above that the SVM2 model performed better than the SVM1 model in the roc_auc, sensitivity, and specificity metrics. This means that the SVM2 model does an significantly better job at accurately separating the data into their respective classes than the SVM1 model.

# Part 4: Prediction

**Find the best probabilities with LDA**
```{r}
lda_mod <- discrim_regularized(frac_common_cov = 0) %>% 
  set_engine("klaR") %>% 
  set_mode("classification")
...
human <- predict(lda_wflow, data.frame(
  animal_name = "human",
  hair = 1,
  feathers = 0,
  eggs = 0,
...
human <- predict(svm_wflow, data.frame(
  animal_name = "human",
  hair = 1,
  feathers = 0,
  eggs = 0,
...
human
```
From the tables above it is evident that the LDA model is able to better predict Mammal as the class type for a human at almost 1.0 while the SVM model predicted Mammal as the class type for human at only 0.37. This makes the LDA model the best model when making predictions with this data.
